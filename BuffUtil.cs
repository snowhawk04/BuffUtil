...
namespace BuffUtil
{
    public class BuffUtil : BaseSettingsPlugin<BuffUtilSettings>
    {
        ...
        private void HandleBladeFlurry()
        {
            ...
            {
                if (showErrors)
                    LogError($"Exception in {nameof(BuffUtil)}.{nameof(HandleBladeFlurry)}: {ex.StackTrace}", 3f);
            }
        }
        ...
        private void HandlePlagueBearer()
        {
            try
            {
                ...
                if (lastPlagueBearerCast.HasValue && DateTime.Compare(currentTime, lastPlagueBearerCast.Value) == -1)
                    return;
